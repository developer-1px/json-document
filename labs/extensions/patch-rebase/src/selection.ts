...
export function prepareSelectionPoint(input: unknown): PreparedSelectionPoint {
  try {
    ...
    return { ok: false, reason: "invalid selection point" };
  }
}
...
function readSelectionPoint(input: unknown): PreparedSelectionPoint {
  if (typeof input === "string") {
    ...
    return path === null
      ? {
          ok: false,
          reason: `invalid selection pointer: ${input}`,
          pointer: input,
        }
  ...
  if (path === null) {
    return {
      ok: false,
      reason: `invalid selection point path: ${rawPath}`,
      pointer: rawPath,
    };
  }
  ...
  if (
    ...
    return { ok: false, reason: "invalid selection point", pointer: path };
  }
