...
import { trackPointer, type JSONPatchOperation } from "@interactive-os/json-document/session";
...
describe("applyPatch — applied field", () => {
  it("returns the input ops on success", async () => {
    const { applyPatch } = await import("@interactive-os/json-document/session");
