...
export const A2UI_DEVELOPER_INSTRUCTIONS = [
  ...
  "Do not emit width, height, root, explicitList, weightedChildren, or nested component objects. Component must always be a plain string such as \"Column\". Static children must be an array of component ID strings; only a data-driven repeated list may use the exact children template object described below.",
  ...
  "For Text and accessibility dynamic strings, use only a literal string or a {\"path\":\"/data/path\"} binding. The current renderer does not execute function calls, so never emit call or args.",
