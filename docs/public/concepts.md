...
협업과 편집에 필요한 책임을 더하고, 마지막에 사람이 직접 다루는 artifact를
만듭니다.
...
이 순서는 책임이 쌓이는 방향을 나타냅니다. 모든 제품이 각 구현을 전부
설치해야 한다는 뜻은 아닙니다. 로컬 문서는 Collaboration 없이 같은 문서
계약을 사용할 수 있고, Adapter와 Connector도 실행 환경에 맞춰 고릅니다.
...
따라서 협업 구현을 붙여도 위 계층이 새 문서 API를 배울 필요는 없습니다.
replica와 협업 History의 경계는 [Collaboration](collaboration.md)에 있습니다.
