...
| JD2-SESSION-001 | insert, replace, delete, move, duplicate, selection, clipboard, history, schema introspection과 그 capability probe는 host 또는 별도 adapter 책임이다. Kernel package의 export나 Projection 적합성에 요구하면 안 된다. |
...
아니다. 별도 `Projection` public type은 외부 구현에서 같은 structural port가
반복되기 전에는 추가하지 않는다.
