...
기존 hit·marquee·drag Affordance와 `createGestureSession`을 조합합니다.
...
Web: hit ID, 같은 좌표계의 point, keyboard stroke, pointer capture
  └─ Affordance: createPlaneSelectProfile
      ...
      └─ preview / commit 결과
          ├─ Hand: 선택 윤곽·marquee·이동 preview 렌더링
          └─ Editing: 선택 반영, 집합 이동·삭제, primary 편집, History
```

### API

- `begin(context, { point, hitKey, shiftKey? })`: `items: { id, x, y, width, height }[]`와
...
- `preview(point)` / `getPreview()`: `{ selection, marquee, translation } | null`.
  `translation`은 `{ keys, dx, dy }` 하나입니다. 문서와 committed selection은 바꾸지 않습니다.
- `commit(point)`: 최종 point로 `{ selection, translation } | null`을 한 번 반환하고
...
- `keyDown(stroke, context, grabbing?)`: selection / delete(keys) / edit(key) / cancel 또는
...
| Shift+객체 drag | press의 toggle 결과로 이동. 누른 대상이 제거되면 이동 없음 |
...
소유합니다. 이 프로파일에는 그룹·다중 resize·중첩 선택·Mod drill-down·축 구속·복제·
snap·zoom/pan·레이어 문법이 없습니다. 기존 단일 Affordance의 더 넓은 기능과 구분합니다.
...
const result = select.commit({ x: 40, y: 50 }); // keys=["node"], dx=20, dy=20
```
