...
test("embedded sheet writes one parent transaction and shares parent Undo, duplication and serialization",()=>{
 ...
 expect(sheet.dispatch({type:"cell.commit",rowId:"r0",columnId:"c0",value:"abc",preserveSelection:true}).ok).toBe(true);
 expect(changed).toHaveBeenCalledTimes(1);expect((sheet.snapshot.value as SheetDocument).rows[0]!.cells.c0).toBe("abc");
 sheet.undo();expect(parent.snapshot.value).toEqual(initial);parent.redo();expect((sheet.snapshot.value as SheetDocument).rows[0]!.cells.c0).toBe("abc");
 ...
 parent.dispatch({type:"object.remove",objectIds:["table"]});expect(sheet.dispatch({type:"cell.commit",rowId:"r0",columnId:"c0",value:"stale"}).ok).toBe(false);
 parent.undo();expect((sheet.snapshot.value as SheetDocument).rows[0]!.cells.c0).toBe("abc");release();
});
